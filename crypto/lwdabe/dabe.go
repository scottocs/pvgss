...
type MAABEPubKey struct {
	Attribs    []string
	EggToAlpha map[string]*bn128.GT
	GToY       map[string]*bn128.G2
}
...
func (a *MAABE) NewMAABEAuth(id string, attribs []string) (*MAABEAuth, error) {
	...
	// generate pubkey
	eggToAlpha := make(map[string]*bn128.GT)
	gToY := make(map[string]*bn128.G2)
	for _, at := range attribs {
		eggToAlpha[at] = new(bn128.GT).ScalarMult(a.Gt, alpha[at])
		gToY[at] = new(bn128.G2).ScalarMult(a.G2, y[at])
	}
...
func (auth *MAABEAuth) PubKeys() *MAABEPubKey {
	newEggToAlpha := make(map[string]*bn128.GT)
	newGToY := make(map[string]*bn128.G2)
	newAttribs := make([]string, len(auth.Pk.Attribs))
	copy(newAttribs, auth.Pk.Attribs)
	for at, gt := range auth.Pk.EggToAlpha {
		newEggToAlpha[at] = new(bn128.GT).Set(gt)
	}
	for at, g2 := range auth.Pk.GToY {
		newGToY[at] = new(bn128.G2).Set(g2)
	}
	return &MAABEPubKey{
		Attribs:    newAttribs,
...
func (auth *MAABEAuth) AddAttribute(attrib string) error {
	...
	y := skVals[1]
	// generate public key
	eggToAlpha := new(bn128.GT).ScalarMult(auth.Maabe.Gt, alpha)
	gToY := new(bn128.G2).ScalarMult(auth.Maabe.G2, y)
	...
	for _, at := range msp.RowToAttrib {
		...
		for _, pk := range pks {
			if pk.EggToAlpha[at] != nil {
				...
				//fmt.Println(lambda[at])
				//lambda[at] = big.NewInt(0).Add(bn128.Order, lambda[at])
				//lambda[at].Mod(lambda[at], bn128.Order)
				if signLambda >= 0 {
...
func (auth *MAABEAuth) GenerateAttribKeys(gid string, attribs []string) ([]*MAABEKey, error) {
	...
	for i, at := range attribs {
		...
		if auth.Sk.Alpha[at] != nil && auth.Sk.Y[at] != nil {
			k = new(bn128.G1).Add(new(bn128.G1).ScalarMult(auth.Maabe.G1, auth.Sk.Alpha[at]), new(bn128.G1).ScalarMult(hash, auth.Sk.Y[at]))
			ks[i] = &MAABEKey{
				Gid:    gid,
				Attrib: at,
				Key:    k,
