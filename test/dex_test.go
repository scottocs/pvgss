...
// 		tx10, _ := dexInstance.Lswap1(auth10, orderId, utils.G1sToPoints(num, lC), utils.G1sToPoints(num, PK1), lsss.Convert(root), utils.IntToBig(I0))
...
// 		auth = utils.Transact(client, privateKeys[0], big.NewInt(0))
// 		tx, _ = dexInstance.Lswap1(auth, orderId, utils.G1sToPoints(num, lC), utils.G1sToPoints(num, PK1), lsss.Convert(root), utils.IntToBig(I0))
// 		receipt, _ = bind.WaitMined(context.Background(), client, tx)
// 		log.Println("On-chain LSwap1 Gas cost = ", receipt.GasUsed)
